...
def data_lists_to_batches(inputList, targetList, batchSize, level):
    ...
    '''
    assert len(inputList) == len(targetList)  # 保证数据feature和label的数量相同 其中len读取的是文件个数
    # dimensions of inputList:batch*39*time_length  time_length就是frame数

    nFeatures = inputList[0].shape[0]
    maxLength = 0
    for inp in inputList:  # find the max time_length
        maxLength = max(maxLength, inp.shape[1])

    # randIxs is the shuffled index from range(0,len(inputList))
    randIxs = np.random.permutation(len(inputList))  # 打乱顺序
    start, end = (0, batchSize)
    dataBatches = []

    while end <= len(inputList):  # batchSeqLengths store the time-length of each sample in a mini-batch
        batchSeqLengths = np.zeros(batchSize)
# randIxs is the shuffled index of input list

        for batchI, origI in enumerate(randIxs[start:end]):  # 如果对一个列表，既要遍历索引又要遍历元素时用，输出index, item
...
def load_batched_data(mfccPath, labelPath, batchSize, mode, level):
    '''returns 3-element tuple: batched data (list), maxTimeLength (int), and total number of samples (int)'''
